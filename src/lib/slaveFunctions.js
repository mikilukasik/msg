module.exports = function slaveFunctionsCreator(msgOptions) {
  ...
  function connectToGateway(address) {
    ...
    if (Object.keys(msgOptions.gateways).indexOf(address) >= 0)
      return log('gateway tried to connect on same route twice: ' + address);
    ...
    thisGateway.ws.on('connect', function (connection) {
      ...
      connection.on('message', function (msg2) {
        ...
        if (message.cmd === 'do') {
          ...
          try {
            ...
            log('ERROR forwarding slave socket:', e.message, e.stack, {
              address,
              cmd,
              message,
              clientSocketRoute,
              publicSocketRoutes: msgOptions.publicSocketRoutes,
            });
          }
          return;
        }
      ...
      try {
        ...
        Object.keys(msgOptions.publicSocketRoutes).forEach(function (route) {
          thisGateway.sendCsRoute(route).then(() => {}, log);
        });

        // send the rules we know of
        Promise.all(
          msgOptions.convertedDirectRules.map(function (rule) {
            return thisGateway.sendRule(rule).then(() => {}, log);
          }),
        ).then(function () {
          log(
            'Number of slave rules sent to MSG on connection (' +
              address +
              '): ' +
              msgOptions.convertedDirectRules.length,
          );
        });
      } catch (e) {
  ...
  function publishRule(rule) {
    if (rule.owner.indexOf(msgOptions.serviceLongName) < 0) {
      ...
      var convertedRule = Object.assign({}, rule, {
        owner,
        outHost: msgOptions.ip.public,
        outPort: msgOptions.port.public,
      });
