    ...
    return new Promise(function (connResolve, rej) {
      ...
      msgOptions.ws.on('connect', function (connection2) {
        ...
        msgOptions.connection.on('close', function () {
          ...
          if (!msgOptions.stopped)
            msgOptions.timeoutIds[`connetionOnCloseRetry-${timeoutIdSuffix}`] = setTimeout(function () {
        ...
        Object.keys(msgOptions.wsRoutes).forEach(function (route) {
          msgOptions.toGtw('wsRoute', route);
        });
        ...
        Promise.all(
          Object.keys(msgOptions.myRules).map(function (key) {
            return msgOptions.sendRule(msgOptions.myRules[key].rule);
          }),
        ).then(function () {
      ...
      function start(onlyStartSocket, secondTry) {
        ...
        try {
          if (!onlyStartSocket) {
            msgOptions.log(
              'Starting ' + msgOptions.serviceName + ' on ' + msgOptions.ip.public + ':' + msgOptions.PORT,
            );
          ...
          const gatewayAddress =
            'ws://' +
            (msgOptions.gatewayAddress ||
              process.env.MSG_ADDRESS ||
              (process.env.MSG_PORT && process.env.MSG_PORT.replace('tcp://', '')) ||
              msgOptions.ip.public) +
            '/sockets';
      ...
      msgOptions
        .getIps('private, public, gateway', { maxTries: 5, optional: 'gateway', ip: msgOptions.ip })
        .then(
          function (ips) {
