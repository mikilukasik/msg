...
module.exports = function createMsgService(optionalOptions) {
  ...
  if (typeof optionalOptions !== 'object')
    throw new Error('msgService string constructor not implemented: ' + optionalOptions);
  ...
  var msgOptions = {
    ...
    ws: new WebSocketClient({
      maxReceivedFrameSize: 160000000,
      maxReceivedMessageSize: 1280000000,
    }),
    PORT: 5000,
    ...
    connection: {
      send: function () {
        log('pure method: connection.send');
      },
    },
    ip: {
      private: '',
  ...
  msgOptions.callWhenConnected = (confirmConnect) => {
    ...
    msgOptions.timeoutIds.callWhenConnected = setTimeout(function () {
      msgOptions.callWhenConnected(confirmConnect);
    }, 200); // TODO: trigger this on connection instead of setTimeout
  };
